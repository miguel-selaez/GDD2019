...
namespace FrbaCrucero.DAO
{
    public class PasajeDAO : BaseDAO<Pasaje>
    {
        ...
        {
            var query = ArmarSentenciaSP("P_Guardar_Pasaje", new[] { GetParam(pasaje.Codigo), GetParam(pasaje.Precio), GetParam(pasaje.Viaje.Id), GetParam(pasaje.Cabina.Id), GetParam(pasaje.Reserva), GetParam(pasaje.Cliente.Id), GetParam(pasaje.Pago) });
